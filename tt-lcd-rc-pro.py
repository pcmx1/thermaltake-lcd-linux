...
Device:   USB HID 264a:232a/233d  →  /dev/hidraw*  (auto-detected)
...
USB_VID = 0x264a
USB_PIDS = (0x233d, 0x232a)   # Thermaltake 3.9" bar/RC Pro LCDs
...
def init_display(fd):
    ...
    if INIT_MODE == 'output':
        ...
        hid_write_control(fd, CMD_18, '18');  time.sleep(0.05)
        hid_write_control(fd, CMD_1A, '1a');  time.sleep(0.05)
        for _ in range(4):
            hid_write_control(fd, CMD_0C_DIMS, '0c');  time.sleep(0.02)
        hid_write_control(fd, CMD_1D, '1d')
...
def main():
    ...
    if data_hidraw is None:
        print(f'ERROR: Thermaltake RC/Bar LCD (USB {supported_usb_ids()}) not found.')
        print('       Check udev rules and that the device is connected.')
        sys.exit(1)

    if CHECK_ONLY:
        print(f'tt-lcd-rc-pro: control={control_hidraw} data={data_hidraw}')
        failed = False
    ...
    if control_hidraw != data_hidraw:
        print(f'tt-lcd-rc-pro: control={control_hidraw} data={data_hidraw}  {W}×{H}  {mode}')
    else:
        print(f'tt-lcd-rc-pro: {data_hidraw}  {W}×{H}  {mode}')
